    ...
    namespace analytics {
        namespace opencv_object_detection {
            ...
            std::shared_ptr<DetectionInternal> convertTrackedObjectToDetection(
                const Frame& frame,
                const cv::detail::tbm::TrackedObject& trackedDetection,
                const std::string& classLabel,
                IdMapper* idMapper);

            DetectionInternalList convertTrackedObjectsToDetections(
                const Frame& frame,
                const cv::detail::tbm::TrackedObjects& trackedDetections,
...
namespace std
{
    template<> struct less<
        const sample::analytics::opencv_object_detection::CompositeDetectionId>
    {
        bool operator()(
            const sample::analytics::opencv_object_detection::CompositeDetectionId&
            lhs,
